...
- **There is no `main`/`master` branch on this repo.** All work happens directly on `claude/*` branches, and the GitHub remote's own default branch is itself a `claude/*` branch. Cloudflare Pages deploys via its own dashboard-configured Git integration (build command `npm run build`, output dir `out` — see Development Workflow above), watching whichever branch is set as its production branch in the Cloudflare dashboard, not something visible from this repo's files (no `wrangler.toml` or `.github/workflows/` exist here). Don't assume a `main` branch exists or try to merge into one without checking first.
