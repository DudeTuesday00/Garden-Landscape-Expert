...
2. **Plantopedia** ("Your Green Thumb Repository") — 10 guide categories, 111 guides total. All 111 are live, indexable, and have full in-depth content — no stub pages remain. Live guides route to a full detail view with sections, tables, tips, callouts, and affiliate product cards.
...
> This tree is a navigational map, not an exhaustive file listing — directories with dozens of near-identical entries (guide content files, plant photo folders) show a representative sample plus a total count rather than every filename. See the "Completed Work" section below for the full history and per-feature detail behind every directory here.
...
│   ├── favicon.png                  # Brand icon — also used as apple-touch-icon and manifest icon
│   ├── og-image.png                 # 1200×630 default social share image (OpenGraph/Twitter)
│   ├── about-me-image.png           # Author photo — About page, AuthorBox on every guide, HomePage E-E-A-T strip
│   ├── data/
│   │   └── hardiness-zones.json     # 40,502-ZIP USDA hardiness zone lookup table (fetched client-side by the Zone Finder tool)
...
│   ├── infographics/                # 29 live infographic images + page header image, all kebab-case filenames
│   ├── videos/
│   │   └── gardening-videos-header.png
│   ├── podcasts/
...
└── src/
    ...
    │   │   └── [guideId]/page.jsx   # /guides/[id]/ — SSG, all 111 guides (generateStaticParams + generateMetadata + Article/Breadcrumb JSON-LD)
    │   ├── plants/
    │   │   ├── page.jsx             # /plants/ — Plant Database browse/search
    │   │   └── [plantId]/page.jsx   # /plants/[id]/ — SSG, all 185 plants
    │   ├── tools/
    │   │   ├── page.jsx             # /tools/ — Garden Tools hub
    │   │   ├── fertilizer-calculator/page.jsx
    │   │   ├── usda-zone-finder/page.jsx
    │   │   ├── gardening-calendar/page.jsx
    │   │   ├── soil-calculator/page.jsx
    │   │   ├── mulch-calculator/page.jsx
    │   │   ├── compost-calculator/page.jsx
    │   │   ├── plant-spacing-calculator/page.jsx
    │   │   ├── yield-estimator/page.jsx
    │   │   ├── plant-symptom-checker/page.jsx
    │   │   ├── companion-planting-checker/page.jsx
    │   │   ├── succession-planner/page.jsx
    │   │   ├── grow-your-own-savings/page.jsx
    │   │   └── watering-calculator/page.jsx
    │   ├── shop/
    │   │   ├── page.jsx             # /shop/ — 3D Printed Garden Shop listing (not linked from nav — see Content & Credibility Improvements)
    │   │   └── [productId]/page.jsx # /shop/[id]/ — SSG product pages
    │   ├── about/page.jsx           # /about/ — About Us / founder bio (E-E-A-T)
    │   ├── contact/page.jsx         # /contact/ — Contact page
    ...
    │   ├── guides-feed.xml/route.js # RSS feed of all 90 live guides — consumed by the separate Newsletter Generator project as Discovery source material
    │   ├── privacy/page.jsx         # /privacy/
    │   ├── affiliate-disclosure/page.jsx
    │   ├── advertising-disclosure/page.jsx
    │   ├── fertilizer-calculator/page.jsx  # Thin noindex redirect stub → /tools/fertilizer-calculator/ (old pre-Tools-hub URL)
    │   ├── not-found.jsx            # Custom 404 page — renders as 404.html on Cloudflare Pages
    │   ├── error.jsx                # Segment-level error boundary
    ...
    │   ├── sitemap.js               # Auto-generates sitemap.xml at build time (guides, plants, products, tools, static pages)
    │   └── robots.js                # Auto-generates robots.txt at build time
    ├── components/
    │   ├── Nav.jsx                  # 'use client' — sticky nav, usePathname active state, dark mode toggle, SiteSearch, hamburger mobile menu
    │   ├── HomePage.jsx             # Landing page — path cards, Topic of the Week, credibility stats strip, E-E-A-T author strip, newsletter signup
    │   ├── TrustBadges.jsx          # Server component — five credential pills (185 Plants, USDA Zones 3–11, 40+ Years, 87 Guides, 100% Free)
    │   ├── TopicOfTheWeek.jsx       # 'use client' — rotating homepage feature card, date-driven via ISO week number
    │   ├── SiteSearch.jsx           # 'use client' — sitewide dropdown search (guides + plants + static pages), lives in Nav
    │   ├── ContactUs.jsx            # 'use client' — contact form (Formspree mlgpgdny)
    │   ├── AboutUs.jsx              # Founder bio / E-E-A-T page content
    │   ├── Infographics.jsx         # 'use client' — 29 live infographic sections, shared lightbox
    │   ├── Videos.jsx               # Curated gardening YouTube channels + original videos
    │   ├── Podcasts.jsx             # Curated gardening podcasts + original Planting Atlas episodes
    │   ├── NewsletterSignup.jsx     # 'use client' — email capture form; Formspree interim
    │   ├── CookieBanner.jsx         # 'use client' — cookie consent bar, localStorage-gated
    │   ├── PrivacyPolicy.jsx / AffiliateDisclosure.jsx / AdvertisingDisclosure.jsx  # Static legal/disclosure pages
    ...
    │   └── tools/                   # ToolsHome.jsx (hub grid) + one subfolder per tool, e.g. tools/soil-calculator/SoilCalculator.jsx
    ├── data/                        # Plain JS data modules — no backend
    │   ├── plants.js                # Core plant database (185 plants across 12 types) — see Plant Schema below
    │   ├── plant-profiles.js        # Companion file: scientific name, lifecycle, native range, toxicity, bloom/harvest fields — keyed by plant id
    │   ├── plant-care-details.js    # Companion file: mature size, growth rate, companions, deer resistance, pollinator value, zone notes, troubleshooting — keyed by plant id
    │   ├── plant-spacing.js         # Per-sq-ft spacing for the Spacing Planner tool
    │   ├── plant-yields.js          # Expected harvest weight/type for the Yield Estimator
    │   ├── planting-windows.js      # Sowing offset weeks (relative to frost dates) for the Planting Calendar
    │   ├── questions.js             # Wizard question definitions (with hydro routing flags)
    │   ├── guides.js                # 10 guide categories, 87 guides — all comingSoon: false
    │   ├── hero-images.js           # Shared heroImages map (guide id → /guides/*.png)
    │   ├── type-guides.js           # Plant type → up to 2 relevant guide links (shared by wizard Results and the Planting Calendar)
    │   ├── topic-of-the-week.js     # Rotation list + optional pinned override for the homepage feature card
    │   ├── tools.js                 # Garden Tools registry — id, name, href, status ('live' | 'coming-soon')
    │   ├── products.js              # 3D printed product database
    │   ├── fertilizer-types.js / fertilizer-recommendations.js   # Fertilizer Calculator catalog + recommendation engine
    │   ├── hardiness-zone-info.js   # Zone → temperature range reference table
    │   ├── frost-date-estimates.js  # Zone → estimated last/first frost date
    │   ├── soil-mixes.js            # Soil & Raised Bed Calculator mix formulas
    │   ├── mulch-types.js           # Mulch Calculator application types + material data
    │   ├── compost-materials.js     # Compost Calculator browns/greens C:N data
    │   ├── companion-pairings.js    # Companion Planting Checker's pairwise good/avoid data — source of truth for plant-care-details.js's companions field on the 25 plants it covers
    │   ├── symptom-key.js           # Garden Symptom Diagnostic's rapid symptom key
    ...
    │   └── guide-content/           # One JS file per live guide + shared index — 111 files total
    ...
    └── logic/                       # Pure functions, unit-tested where noted
        ...
        ├── topicOfWeek.js           # ISO week math + rotation/pin resolution (tested — topicOfWeek.test.js)
        ├── plantingCalendar.js      # Frost-date-relative sowing/harvest month computation
        ├── soilCalculator.js / mulchCalculator.js / compostCalculator.js / spacingPlanner.js / yieldEstimator.js / successionPlanner.js / growYourOwnSavings.js / wateringCalculator.js   # One module per Garden Tools calculator
```
...
- **SSG:** Dynamic routes pre-rendered at build time via `generateStaticParams` — one HTML file each for all 111 guides, 185 plants, and 6 shop products (~322 static pages total from a clean `next build`)
...
- **Ads:** Google AdSense (`ca-pub-2083020536499662`) — **currently disabled**; the loader script is gated behind `NEXT_PUBLIC_ENABLE_ADSENSE === 'true'` in `layout.jsx` pending Google's approval (see "Notes for AI Assistants" at the end of this file)
- **Database:** Static JS files (no backend)
- **Testing:** Vitest — `npm test` runs `vitest run`; covers `src/logic/matchPlants.js` and `src/logic/topicOfWeek.js` (24 tests total)
...
- **Newsletter Generator — Phase 2: automated subscriber email (the last open roadmap item).** The generator itself (discovery → draft → approve → publish) is live — see "Newsletter Generator ✅" under Completed Work. The subscriber-capture side is also built but deactivated — see "Newsletter Subscriber Capture — n8n Workflow" under Completed Work: an n8n webhook + Data Table exist, validated with 0 errors, but the site's `NewsletterSignup.jsx` still posts to the Formspree interim endpoint. Two owner-side blockers stand between here and live: (1) a **Cloudflare Tunnel** to expose the n8n webhook publicly (it's LAN-only today), and (2) **SMTP/transactional-email credentials** for the not-yet-built Send-to-Subscribers workflow. Neither can be supplied from a coding session — both need the owner's direct action.
- **Planting Guides — complete.** All 87 guides are live, indexable, and fully expanded — the stub-guide backlog (Item 1 of the roadmap) is finished as of 2026-09-09. Any future new guide subject should be authored as a full guide from the start using the established pattern in `guide-content/`, either from a source `.docx` when the owner has one or authored directly from compiled horticultural knowledge when one isn't available (see "Notes for AI Assistants" for the authoring standard).
- Real product photography for the 3D Printed Garden Shop (`/shop/` and the nav's Shop link both already point at the real Etsy storefront — see "Site Improvement Roadmap — Items 0, 3, 4, 5, 6, 7, 8" and "Etsy Shop Link" under Completed Work; this item is only about the 6 placeholder products' photos, not visibility)
- Google AdSense re-enablement once the site is approved (currently gated off — see Tech Stack above)
- Google Ads conversion tracking is scaffolded and inert (`NEXT_PUBLIC_GOOGLE_ADS_ID` unset) — activate once the owner has a real Google Ads account with a Conversion ID and per-action labels
...
### SEO Fixes: Real Dates, Live Counts, Title Length, Page Schema ✅ (2026-09-25)

Follow-up to the SEO audit. **Workflow change: run `npm run dates` before committing any guide/tool/page content change** — it regenerates `src/data/content-dates.json` from git history (`scripts/generate-content-dates.mjs`); the file is committed because Cloudflare builds may use a shallow clone. A vitest guard (`src/data/site-stats.test.js`) fails if the guide count/category count or any live guide's dates are missing or stale.

- **Honest dates:** guide Article JSON-LD `datePublished`/`dateModified`, the visible byline ("Updated September 2026"), and every sitemap `lastmod` now come from git (first-added / last-commit date of the guide content file; tools by their component folder; static pages by their component; plants by the plant data files). The old hardcoded 2026-03-01 / 2026-04-05 dates and the "Updated March 2026" byline are gone. Tool and wizard dates deliberately ignore route `page.jsx` files, since metadata-only edits would falsely mark everything modified. The sitemap now lists live tools from `tools.js` automatically.
- **Live guide count:** "75+" copy (layout, homepage, guides index metadata, homepage text) and the hand-typed "114/117/120" values (homepage stats, trust badges, search index) now read `src/data/site-stats.js` (from content-dates.json, so client bundles do not import the whole guide list).
- **Title length:** all 120 guide titles were 61–82 characters with ugly mid-phrase cuts; now every title in guides, plants, tools, and newsletters is at most 60 characters including " | Planting Atlas". `src/logic/seoTitle.js` (`fitTitle`, tested) cuts at a colon/dash/parenthesis, otherwise at a word boundary without dangling words; guides use "title — subtitle" only if it fits in 43 characters, else the title alone; plants show the scientific name only if it fits; long newsletter titles use an absolute title with an ellipsis and skip the suffix.
- **Page schema:** every plant page gets `WebPage` (+ `about` Thing with scientific name) and `BreadcrumbList`; every tool page gets `WebApplication` (free, `LifestyleApplication`) and `BreadcrumbList`, and `/tools/` gets `CollectionPage` + `ItemList`, all via one shared `src/app/tools/layout.jsx` + `ToolJsonLd.jsx` (keys off `tools.js`, so new tools are covered automatically).
- Not done: `dateModified` on plant pages is one database-wide date, not per plant; the newsletter pipeline still generates issue titles that are long (now safely shortened for search).
...
- **Plant-page section** (`PlantDetail.jsx`, new "💧 Growing X Hydroponically" card on all 27 `hydroponic: true` plants): time to harvest, pH, and — only where the site has a sourced range — nutrient strength (EC) and light (DLI); best-suited and workable systems, each linking to its guide; the plant's own `hydroponicsNotes`; and tool/guide links. Data: `src/data/hydroponic-plant-targets.js` (plant → crop group + `exact`/`group` basis, `systemGuideHref`), logic `src/logic/hydroponicPlantProfile.js` (+ `.test.js`, 7 tests; suite now 54). **Honesty rule:** only lettuce, basil, tomato, hot pepper, bell pepper, and cucumber get exact targets (they are literally rows in `hydroponic-targets.js`); other leafy greens/herbs show group ranges explicitly labeled "typical for leafy greens as a group"; radish, broccoli, eggplant, strawberry, rosemary, thyme, oregano, and stevia get NO EC/light target (no reliable range) and say so. Plant meta descriptions for hydroponic plants also gain "Also suited to hydroponic growing."
- **RSS feed** `/hydroponics-feed.xml` (`src/app/hydroponics-feed.xml/route.js`, data in `src/data/hydroponics-hub.js`): the 12 hydroponics guides + 4 tools + any newsletter issue in the hydroponics series or clearly about soilless growing (`isHydroponicsNewsletter`), newest first, real pubDates from git history, `atom:link rel=self`. Hydroponic guide pages emit `<link rel="alternate" type="application/rss+xml">` autodiscovery. Adding a hydroponics guide/tool = add it to `hydroponics-hub.js`.
- **Newsletter series "Hydroponics From Scratch"** (lives in `D:\Planting Atlas Newsletter`, not this repo): 6 parts (basics/system choice → Kratky jar → nutrients+pH+EC → light → tomatoes & peppers in buckets → troubleshooting), each grounded in a live guide. Series registered in `config/newsletter-plan.json` (`activeSeriesId` set; backup at `newsletter-plan.json.bak-pre-hydroponics`), six topics seeded into the review-app queue at scores 100→95 so the daily 10:00 CT generator takes Part 1..6 in order, and the `Planting Atlas - Hydroponics` feed added to the source registry **disabled until this deploys**. See that project's CLAUDE.md for undo instructions.

### Three More Guides — Potatoes, Garlic & Onions, Roses ✅ (2026-09-25)

Continuing the gap-fill (no dedicated guide existed for any of these): `growing-potatoes` (Edible Gardens — seed potatoes + chitting, planting/hilling with a green-tuber toxin warning, containers/grow bags, problems table, harvest/curing/storage), `growing-garlic-onions` (Edible Gardens — hardneck vs softneck, onion day-length table, care + problems, harvest/curing; defers the fall-garlic timing window to the Fall Planting Guide), `growing-roses` (Flowers & Color — types by care level, site/planting, watering/feeding, pruning by bloom habit, problems table; defers technique to Pruning Basics). Themes: amber/stone, purple/fuchsia, rose/pink. Guide count **111**.

- Numbers are hedged (soil temps, storage 40–45°F potatoes / 50–60°F curing, onion latitude bands ~32° and ~35–36° N, rose pH 6.0–6.5).
- **Editing lesson:** guides.js/index.js/hero-images.js/GuideDetail.jsx are now edited with the Edit tool, not Python heredocs — Python string handling silently collapsed backslash-escaped apostrophes twice and broke builds. The verification gate scans every guide file with `node --check`, reads BUILD_EXIT and the test count from one log, and only then commits.

### Three More Guides — Peppers, Squash & Zucchini, Hydrangeas ✅ (2026-09-25)

Chosen by comparing existing guide coverage against newsletter-archive topic frequency (all three had newsletters but no dedicated guide): `growing-peppers` (Edible Gardens — seed starting with bottom heat, warmth/timing, heat-level table with hedged Scoville ranges, feeding/support + problems table, harvest/storage; leans on Growing Tomatoes and Starting Seeds Indoors), `growing-squash-zucchini` (Edible Gardens — summer vs winter, planting, male/female pollination, pests table incl. vine borer and squash bugs, harvest/curing/storage with acorn-vs-butternut curing distinction), `hydrangeas` (Flowers & Color — five types table with hardiness and old/new wood, sun/soil/water, color-change table with pH ranges and an over-application warning, pruning timing table, troubleshooting + winter care; defers general technique to Pruning Basics). Themes: yellow/orange, lime/yellow, blue/violet. Guide count **108**.

- Figures are hedged ranges (Scoville, hardiness zones, curing temperature ~80–85°F then storage ~50–55°F, pH 5.2–5.5 blue / 6.0–6.2 pink).
- Process: syntax-checked every touched module, and committed only after reading the real build exit code and test count (see the previous entry's process lesson).

### Three More Guides — Starting Seeds Indoors, Pruning Basics, Home Greenhouse ✅ (2026-09-25)

General (non-hydroponics) gap-fillers, chosen because none existed as dedicated guides and the newsletter archive shows demand (pruning tomatoes/hydrangeas, greenhouse building): `starting-seeds-indoors` (Edible Gardens — mix/containers, sowing + germination temps table, light as the #1 failure, bottom watering/damping off, potting up + hardening off; cross-links Spring Startup, Grow Lights, Cold Frames rather than repeating their timing tables), `home-greenhouse-guide` (Edible Gardens — types + coverings tables, siting, ventilation/shade/heat, season-by-season crops, pests table), `pruning-basics` (Landscape Design — tools, the three cuts, old-wood vs new-wood timing table, shrubs/roses/hedges, mistakes table; defers plant-specific timing to Winter Garden Prep and fruit pruning to Fruit Trees). Themes: lime/green, emerald/teal, slate/emerald. Guide count **105**.

- Figures are stated as rules of thumb and hedged (vent area ~15–20% of floor area, shade cloth 30–50%, prune-no-more-than-a-quarter-to-a-third, oak-wilt timing "check local extension guidance").
- **Process lesson (a commit slip):** an earlier batch was committed with a broken build because the command chain ran the build and tests but did not gate the commit on their exit codes — the cause was an unescaped apostrophe (`chair's`) in a single-quoted string in `guides.js`, introduced by a Python heredoc that collapsed `\'` to `'`. Rules now: (1) never put apostrophes in `guides.js` descriptions, (2) run `node --check` on every touched `.js` module before building, (3) commit only after the build exits 0 **and** vitest shows the full test count, in an `&&` chain, and (4) do not delete the build log until the result has been read.

### Hydroponics Guides, Batch 4 — Vertical Towers, Wick Systems, Best Plants ✅ (2026-09-24)

Final planned hydroponics guides (Specialty Gardens): `vertical-hydroponic-towers` (how towers work, what to grow, setup with lighting-per-level caveat, maintenance table), `wick-system-hydroponics` (capillary mechanics, build, crop fit, salt-buildup troubleshooting), `best-plants-for-hydroponics` (what makes a good hydroponic crop; tables for greens, herbs, fruiting crops, and poor fits). The plants guide's harvest times and system pairings are taken directly from `plants.js` `daysToHarvest` for the 27 `hydroponic: true` plants, so it matches the Plant Database and the wizard. Themes: fuchsia/purple, stone/amber, green/emerald. 5 images + hero each. The System Chooser now links every one of its 7 systems to a guide and links out to Best Plants.

- **Hydroponics push status:** system chooser + 4 calculators/tools, and 12 guides (Beginners, Kratky, Nutrients/pH/EC, DWC, NFT, Grow Lights, Ebb & Flow, Dutch Bucket, Troubleshooting, Vertical Towers, Wick, Best Plants). Guide count **102**. Next step per the plan is not more content but measuring — check Search Console impressions on these pages before building further (per-plant hydroponic sections on plant pages, a hydroponics RSS feed, a newsletter series, and real grow logs with measured EC/pH remain unbuilt).

### Hydroponics Guides, Batch 3 — Ebb & Flow, Dutch Bucket, Troubleshooting ✅ (2026-09-24)

Third hydroponics batch (Specialty Gardens): `ebb-and-flow-hydroponics` (flood/drain mechanics, parts + overflow-first setup, flood-schedule guidance framed as adjust-to-the-plants, crop fit, troubleshooting), `dutch-bucket-hydroponics` (per-plant buckets with elbow drain reservoir, recirculating vs run-to-waste, build, crop fit, feeding with the same EC/pH ranges as the Nutrients guide, troubleshooting), `hydroponic-troubleshooting` (cross-system: diagnostic order, root-reading table, leaf-symptom table, water + pest table, equipment failure by system). Themes: teal/sky, orange/amber, rose/pink. 5 inline images + hero each.

- The System Chooser now links its top match to the Ebb & Flow and Dutch Bucket guides too (every system with a guide links to it: Kratky, DWC, NFT, ebb & flow, Dutch bucket).
- Flood frequency/duration, bucket sizes, and similar figures are deliberately stated as rules of thumb to adjust by watching the plants. The troubleshooting guide is hedged ("possible cause") and does not give peroxide/pH-adjuster doses.
- Guide count is now **99**. Remaining hydroponics guides from the plan: vertical towers, wick systems, best plants for hydroponics.
...
`src/data/guide-content/growing-tomatoes.js`, `raised-bed-gardening.js`, and `growing-berries.js` — three brand-new full guides added to Edible Gardens, selected as genuine content gaps rather than expanding an existing stub (the stub backlog was already fully cleared as of the previous batch). Tomatoes are the single most popular home-garden vegetable and had no dedicated guide (only partial mentions across `salsa-garden`, `pizza-garden`, `container-vegetables`); raised beds and small fruit/berries were both common, high-relevance topics with no coverage at all.

| Guide | Sections | Notes |
|---|---|---|
| Growing Tomatoes: The Complete Guide (`id: 'growing-tomatoes'`) | 5 sections | Starting from seed, determinate/indeterminate variety selection table, planting deep + staking methods table, watering/feeding for blossom-end-rot and cracking prevention, common problems table |
| Raised Bed Gardening (`id: 'raised-bed-gardening'`) | 5 sections | Why raised beds, materials/construction lifespan table, sizing and height rules, filling with a soil mix, placement and first-season care |
| Growing Berries: Strawberries, Blueberries & Brambles (`id: 'growing-berries'`) | 5 sections | Strawberry fruiting-type planting systems table, blueberry soil-pH requirements, raspberry/blackberry bearing-type pruning table, site/soil prep, harvest and bird-netting care |

- All three cross-reference existing guides/tools rather than duplicating them: `growing-tomatoes` → Vegetables in Containers, Organic Pest Control, Common Garden Diseases, Post-Harvest Garden Care; `raised-bed-gardening` → Square Foot Gardening (explicitly scoped as "this guide covers the bed itself, that one covers what's grown inside it") and the Soil & Raised Bed Calculator tool; `growing-berries` → Fruit Trees (explicitly distinguished as a different plant category), Soil Health, Organic Pest Control.
- Added to `guides.js` under the Edible Gardens category (9 → 12 guides in that category)
- New theme entries in `GuideDetail.jsx`: `growing-tomatoes` (red/orange gradient), `raised-bed-gardening` (amber/yellow gradient), `growing-berries` (indigo/purple gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline used throughout the site. One image (`raised-bed-gardening`'s first section image) needed a regenerate — the first attempt showed empty unplanted beds, which mismatched the section's "beds filled with thriving vegetable plants" framing; reprompted for full, planted beds from a different angle than the hero, which fixed it. All 18 final images manually reviewed before commit.
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in all three guides' built HTML
- Guide counts updated everywhere: **90 full guides, 0 stubs** (was 87/0) — `HomePage.jsx`, `TrustBadges.jsx`, `manifest.json`, and `search-index.js` all updated to "90".

### Site Improvement Roadmap — Items 0, 3, 4, 5, 6, 7, 8 ✅ (2026-09-06)

Six of the seven items from the site improvement roadmap (everything except guide-content expansion and the n8n newsletter pipeline, both tracked separately as larger multi-session work) landed in one session, `a728640`. This entry backfills documentation that was missed at the time — the commit itself is real and already on `main`, verified via a clean production build and live click-through testing.

- **Item 0 — docx policy removed.** The "a guide must not be built unless a `.docx` source exists" hard requirement is gone from CLAUDE.md's "Notes for AI Assistants" — replaced with the compiled-knowledge authoring standard already used for `plant-care-details.js`, `companion-pairings.js`, etc. This was the standing blocker that gated Item 1's entire 16-batch stub-guide expansion.
- **Item 5 — Shop → Etsy.** `src/components/shop/BuyOnEtsyButton.jsx` (`'use client'`, fires `trackEvent('shop_etsy_click', {product_id})`) replaces the old disabled "Add to Cart — Coming Soon" button in `ProductDetail.jsx`. `products.js` gained a per-product `etsyUrl` field (falls back to `SHOP_ROOT = 'https://www.etsy.com/shop/PixelsandChisels'` when unset). `ShopHome.jsx` gained an Etsy banner/CTA so `/shop/` reads as a catalog for the real storefront rather than an incomplete built-in cart.
- **Item 3 — Square PWA icon.** `public/icon-512.png`, `public/icon-192.png`, `public/apple-touch-icon.png` generated from the existing (non-square, 435×637) `favicon.png` art. `manifest.json`'s `icons` array now lists all three square sizes alongside the original; `layout.jsx`'s `metadata.icons.apple` points at the new 180×180 file.
- **Item 4 — GA4 tracking extensions.** All via the existing `trackEvent()` utility in `src/logic/analytics.js`: `affiliate_click` (`src/components/guides/AffiliateLink.jsx`, new — `{guide_id, product}`), `shop_etsy_click` (`BuyOnEtsyButton.jsx` above, `{product_id}`), `newsletter_signup` (one-line addition to `NewsletterSignup.jsx`), and scroll-depth events `guide_read_50`/`guide_read_90` — a new `src/logic/useReadDepthTracking.js` hook (`IntersectionObserver` on two sentinel `<div id="read-depth-50/90">` elements positioned at 50%/90% through a guide's sections) wrapped by `src/components/guides/ReadDepthTracker.jsx` and mounted once per guide in `GuideDetail.jsx`.
- **Item 6 — Google Ads conversion scaffolding.** `src/logic/analytics.js` gained `reportConversion(conversionLabel, params)`, no-op until `NEXT_PUBLIC_GOOGLE_ADS_ID` is set (see `.env.example`). `layout.jsx` conditionally emits a second `gtag('config', ...)` call alongside the existing GA4 init when the env var is present — same inert-until-configured pattern already used for AdSense.
- **Item 7 — "My Garden."** `src/logic/useSavedItems.js` (`localStorage` key `pa-my-garden`, SSR-safe hydration guard matching `CookieBanner.jsx`'s pattern) + `src/components/SaveButton.jsx` (heart/bookmark toggle, `type: 'plant' | 'guide'`) + `src/components/MyGarden.jsx` and `src/app/my-garden/page.jsx` (client-rendered list page, `noindex` since it's per-browser personal state with no unique content for crawlers). `SaveButton` embedded in both `GuideDetail.jsx` and `PlantDetail.jsx`; `❤️ My Garden` added to `Nav.jsx`. Save/persist/list/remove round trip confirmed live via real clicks on both a plant and a guide page, not just a clean build.
- **Item 8 — Printable guides.** `src/components/guides/PrintGuideButton.jsx` (`window.print()`, wrapped in its own `print:hidden`) added to the share-button row in `GuideDetail.jsx`. `print:hidden` applied to everything that shouldn't print: the back-to-guides link, the Save-to-My-Garden button, the share-button row itself, the three AdSense placeholder divs, the newsletter signup, the author box, and the related-guides grid — reusing the exact print pattern already proven on the wizard's `ShareBar`.

**Not done in this batch — Item 2 (Newsletter n8n subscriber capture) is the one remaining roadmap item**, and it's already partially built — see "Newsletter Subscriber Capture — n8n Workflow" above: the capture workflow and Data Table exist but are deactivated, blocked on owner-side infrastructure (a Cloudflare Tunnel to expose the webhook publicly, and SMTP/transactional-email credentials for the not-yet-built Send-to-Subscribers workflow) that can't be supplied from a coding session.

### Overwintering & Year-Round Calendar Guides Expanded to Full Content ✅ (2026-09-09)

`src/data/guide-content/overwintering.js` and `src/data/guide-content/year-round-calendar.js` expanded from 2-paragraph stubs to full guides — the sixteenth and **final** stub-guide batch (Item 1). **All 87 Plantopedia guides now have full in-depth content — the stub-guide backlog is complete.**

| Guide | Sections | Notes |
|---|---|---|
| Overwintering Tender Plants (`id: 'overwintering'`) | 5 sections | Which plants need lifting (dahlia/canna/gladiolus/elephant ear/caladium/tuberous begonia table), digging timing and curing, storage conditions by plant table, mid-winter checks, spring division and reawakening |
| Year-Round Gardening Calendar (`id: 'year-round-calendar'`) | 5 sections | How zone-shifted timing works, winter planning months, spring/summer vegetable task calendar table, fall task calendar table, building your own zone-specific calendar |

- These two carried the heaviest content-overlap risk of the whole 16-batch series, flagged explicitly in the prior batch's note. Resolved by narrowing each to the specific angle its overlapping guide doesn't already cover: `overwintering` focuses specifically on dig-and-store mechanics for dormant tubers/corms/rhizomes (dahlia, canna, gladiolus, elephant ear, caladium, tuberous begonia), explicitly distinct from Winter Garden Prep's existing "Overwintering Tender Plants Indoors" section, which covers actively-growing container tropicals (geraniums, fuchsias, hibiscus) kept alive indoors rather than dormant storage — cross-referenced directly. `year-round-calendar` focuses specifically on the vegetable/edible garden task calendar (seed starting, direct sowing, transplanting, succession), explicitly distinct from Four-Season Garden Design's existing "Month-by-Month Garden Calendar" section, which is ornamental-bloom-sequence-focused — cross-referenced directly, plus cross-references to Fall Planting and Post-Harvest Garden Care for their respective detail.
- New theme entries in `GuideDetail.jsx`: `overwintering` (indigo/slate gradient), `year-round-calendar` (teal/cyan gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline. Three images needed regeneration: `overwintering`'s storage-boxes image first showed generic shredded packing material with no visible tubers (content mismatch, not a labeled-object issue) — reprompted to explicitly show tubers nestled in the vermiculite, which fixed it. `year-round-calendar`'s hero and planning-notebook images both hit the labeled-object gibberish-text lesson again — a seed packet with implied readable branding/text produced garbled pseudo-words ("HubUA", "SOCEKE") — fixed by reprompting with plain/blank/unlabeled packets and envelopes instead. `year-round-calendar`'s seed-starting image showed mature onion bulbs instead of seedling sprouts (content mismatch) — reprompted for thin grass-like seedling sprouts in trays, which fixed it. `year-round-calendar`'s garden-journal image showed handwriting styled as an attempted legible numbered list — reprompted with the notebook pages intentionally out of focus (sharp foreground trowel/pot instead), the same "reframe so the marked surface isn't implied legible" fix already documented elsewhere in this file. All 12 final images manually reviewed before commit.
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: **87 full / 0 stubs** (was 85/2) — the Plantopedia stub-guide backlog (Item 1 of the roadmap) is now complete. `HomePage.jsx`'s stat strip updated to "87 In-depth guides".
...
`src/data/guide-content/spring-startup.js` and `src/data/guide-content/summer-maintenance.js` expanded from 2-paragraph stubs to full guides — the fourteenth stub-guide batch (Item 1). These two were deliberately chosen next from the remaining 6 seasonal stubs because they scope cleanly as task-sequencing/timing guides, distinct from the plant-selection-focused Four-Season Garden Design guide and the fall-specific Winter Garden Prep guide.

| Guide | Sections | Notes |
|---|---|---|
| Spring Garden Startup (`id: 'spring-startup'`) | 5 sections | Correct task order/sequence, soil temperature benchmarks by crop table, indoor seed starting timing table, hardening off, zone-by-zone startup windows table |
| Summer Garden Maintenance (`id: 'summer-maintenance'`) | 5 sections | Deep watering technique, deadheading/cutting back by plant table, pest/disease scouting habit, succession planting, heat stress management table |

- Both cross-reference existing guides rather than duplicating them (Water-Wise Gardening for irrigation detail, Organic Pest Control for IPM treatment, Cold Frames for hardening-off environment, Succession Planting Planner tool).
- New theme entries in `GuideDetail.jsx`: `spring-startup` (green/lime gradient), `summer-maintenance` (yellow/orange gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline. One image (soil thermometer dial face) needed two regenerate attempts — dial gauges with numeric markings reliably produce garbled/gibberish numerals (the same labeled-object lesson noted elsewhere in this file); fixed by reframing the shot so the dial is held at an angle where the numbers are naturally out of focus/illegible rather than a clear flat-on readable face. All 12 final images manually reviewed before commit (no artifacts).
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 83 full / 4 stubs (was 81/6)
- Only 4 stub guides remain: fall-planting, overwintering, post-harvest, year-round-calendar — each carries real overlap risk with existing full guides (Winter Garden Prep, Seed Saving, Spring Bulb Garden, Four-Season Garden Design) and will need the same careful scoping/cross-referencing treatment applied here before expansion.

### Aromatherapy Garden & Fire-Safe Landscaping Guides Expanded to Full Content ✅ (2026-09-08)

`src/data/guide-content/aromatherapy-garden.js` and `src/data/guide-content/fire-safe-landscaping.js` expanded from 2-paragraph stubs to full guides — the thirteenth stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Aromatherapy Garden (`id: 'aromatherapy-garden'`) | 5 sections | Plants by therapeutic effect table, seating/shelter/airflow for scent capture, seasonal scent succession table, container herbs, complete wellness garden layout |
| Fire-Safe Landscaping (`id: 'fire-safe-landscaping'`) | 6 sections | Three-zone defensible space framework table, ember-driven ignition risk, plant flammability ratings table, low-flammability alternatives table, hardscape buffer design, regional requirements table |

- New theme entries in `GuideDetail.jsx`: `aromatherapy-garden` (purple/violet gradient), `fire-safe-landscaping` (orange/red gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 81 full / 6 stubs (was 79/8)
- Only 6 stub guides remain: fall-planting, overwintering, post-harvest, spring-startup, summer-maintenance, year-round-calendar — all seasonal-calendar-themed and carrying real overlap risk with the already-full Four-Season Garden Design and Winter Garden Prep guides; expanding these will need careful scoping to avoid duplication, not just a straightforward compiled-knowledge write-up.

### Pet-Friendly Plants & Sensory Garden Guides Expanded to Full Content ✅ (2026-09-08)

`src/data/guide-content/pet-friendly-plants.js` and `src/data/guide-content/sensory-garden.js` expanded from 2-paragraph stubs to full guides — the twelfth stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Pet-Friendly Plants (`id: 'pet-friendly-plants'`) | 5 sections | Most dangerous common garden plants table, using the ASPCA database, safe alternatives by garden use table, lawn/turf safety (cocoa mulch warning), pet-friendly design strategies |
| Sensory Garden (`id: 'sensory-garden'`) | 6 sections | Plants for fragrance/texture/sound/taste tables, accessible design considerations, layering multiple senses in a small space |

- New theme entries in `GuideDetail.jsx`: `pet-friendly-plants` (amber/lime gradient), `sensory-garden` (violet/fuchsia gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 79 full / 8 stubs (was 77/10)

### Cottage Landscape & Low-Maintenance Landscape Guides Expanded to Full Content ✅ (2026-09-08)

`src/data/guide-content/cottage-landscape.js` and `src/data/guide-content/low-maintenance-landscape.js` expanded from 2-paragraph stubs to full guides — the eleventh stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Cottage Landscape Design (`id: 'cottage-landscape'`) | 5 sections | Design principles behind controlled abundance, fences/paths/climbing structures table, regional climate adaptation table, working with self-seeders table, small-space application |
| Low-Maintenance Landscape (`id: 'low-maintenance-landscape'`) | 5 sections | Right-plant-right-place selection, lawn reduction strategies table, mulching systems, drought-tolerant plant palettes table, edging/hardscape |

- `cottage-landscape` deliberately scoped to the structural/design side (fences, paths, regional adaptation, self-seeders) with a cross-reference to the existing Cottage Garden Style guide for the deeper plant-by-plant palette, avoiding duplication of that guide's content.
- `low-maintenance-landscape` cross-references the existing Native Plants and Water-Wise Gardening guides rather than re-deriving their plant lists and irrigation detail.
- New theme entries in `GuideDetail.jsx`: `cottage-landscape` (pink/rose gradient), `low-maintenance-landscape` (slate/emerald gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 77 full / 10 stubs (was 75/12)

### Vertical Gardening & Foundation Planting Guides Expanded to Full Content ✅ (2026-09-08)

`src/data/guide-content/vertical-gardening.js` and `src/data/guide-content/foundation-planting.js` expanded from 2-paragraph stubs to full guides — the tenth stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Vertical Gardening (`id: 'vertical-gardening'`) | 6 sections | Trellis types by growth habit table, vining vegetables table, ornamental climbers table, tower/pocket planters, living wall systems, designing a vertical system |
| Foundation Planting Guide (`id: 'foundation-planting'`) | 6 sections | Sizing and spacing rules, evergreen/deciduous mix table, corner specimens vs. mass planting, difficult conditions table (shade, reflected heat, dry overhang, wet zones), moisture management, regional recommendations table |

- New theme entries in `GuideDetail.jsx`: `vertical-gardening` (lime/teal gradient), `foundation-planting` (stone/green gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 75 full / 12 stubs (was 73/14)

### Native Plants & Water-Wise Gardening Guides Expanded to Full Content ✅ (2026-09-08)

`src/data/guide-content/native-plants.js` and `src/data/guide-content/water-wise-gardening.js` expanded from 2-paragraph stubs to full guides — the ninth stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Native Plants Guide (`id: 'native-plants'`) | 5 sections | Ecological case for natives table, sourcing local ecotypes (with a nativar caution), native plants by US region table, water/maintenance benefits, transitioning a conventional landscape |
| Water-Wise Gardening (`id: 'water-wise-gardening'`) | 6 sections | Drip irrigation design, mulching strategies table, hydrozoning table, deep vs. shallow watering technique, soil amendment for retention, rainwater harvesting table |

- `water-wise-gardening` deliberately scoped to general water-saving techniques any garden can apply, cross-referencing the existing Xeriscape Design guide (which already covers a full ground-up low-water landscape redesign with detailed regional plant tables) and the Rain Barrel guide, rather than duplicating either.
- New theme entries in `GuideDetail.jsx`: `native-plants` (emerald/amber gradient), `water-wise-gardening` (sky/emerald gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 73 full / 14 stubs (was 71/16)

### Cold Frames & Composting Basics Guides Expanded to Full Content ✅ (2026-09-07)

`src/data/guide-content/cold-frames.js` and `src/data/guide-content/composting-basics.js` expanded from 2-paragraph stubs to full guides — the eighth stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Cold Frames & Season Extension (`id: 'cold-frames'`) | 5 sections | Construction and sizing, siting/ventilation (with an overheating warning — more crops are lost to overheating than frost), crop selection table, row cover/low tunnel/hoop house comparison table, timing by zone |
| Composting Basics (`id: 'composting-basics'`) | 6 sections | Browns/greens ratio, hot vs. cold composting table, what to compost/avoid, troubleshooting table, vermicomposting, applying finished compost |

- `composting-basics` deliberately scoped as the dedicated practical "how to compost" guide, cross-referencing Soil Health and Organic Fertilizing (which already cover compost as one part of broader soil/fertilizing strategy) rather than duplicating them.
- New theme entries in `GuideDetail.jsx`: `cold-frames` (sky/cyan gradient), `composting-basics` (stone/amber gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline. One image (`cold-frames` propped-lid) needed a regenerate — the first attempt showed a decorative stick resting on a mostly-closed lid rather than actually propping it open for ventilation; reprompted explicitly for the lid lifted and wedged open, which fixed it. All 12 final images manually reviewed before commit (no artifacts).
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 71 full / 16 stubs (was 69/18)

### Bird Garden & Water Feature Plants Guides Expanded to Full Content ✅ (2026-09-07)

`src/data/guide-content/bird-garden.js` and `src/data/guide-content/water-feature-plants.js` expanded from 2-paragraph stubs to full guides — the seventh stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Bird-Friendly Garden (`id: 'bird-garden'`) | 6 sections | Berry-producing shrubs table, seed-bearing perennials/fall cleanup timing, water features and shelter, nest box specifications table by species, pesticide-free pest management, year-round food availability table |
| Water Feature Plants (`id: 'water-feature-plants'`) | 6 sections | Plant categories by water depth table, balancing the ecosystem, pond/bog design principles, native vs. non-native considerations table (including an invasive-species warning), winterizing aquatic plants table, container water gardens |

- New theme entries in `GuideDetail.jsx`: `bird-garden` (sky/amber gradient), `water-feature-plants` (cyan/blue gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 69 full / 18 stubs (was 67/20)

### Cutting Garden & Fragrant Garden Path Guides Expanded to Full Content ✅ (2026-09-07)

`src/data/guide-content/cutting-garden.js` and `src/data/guide-content/fragrant-garden-path.js` expanded from 2-paragraph stubs to full guides — the sixth stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Cutting & Drying Garden (`id: 'cutting-garden'`) | 5 sections | Three-layer system table (annual/perennial/everlasting), succession planting schedule, harvesting technique for vase life, drying methods by flower type table, small-space design |
| Fragrant Garden Path (`id: 'fragrant-garden-path'`) | 5 sections | Foot-traffic-tolerant plant table, stepping-stone spacing, upright brush-by border plants table, fragrance layering by season table, sun vs. shade design |

- Deliberately differentiated `cutting-garden` from the already-published `cut-flower-garden` guide by focusing specifically on succession scheduling and drying technique (with an explicit cross-reference), and `fragrant-garden-path` from `plants-for-fragrance` by focusing on walkway/foot-traffic design rather than a general fragrant-plant catalog.
- New theme entries in `GuideDetail.jsx`: `cutting-garden` (rose/purple gradient), `fragrant-garden-path` (violet gradient) — caught and fixed a non-existent Tailwind class (`lavender-100`, not a real Tailwind color) before it could silently fail to render.
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 67 full / 20 stubs (was 65/22)

### Hanging Baskets & Container Vegetables Guides Expanded to Full Content ✅ (2026-09-07)

`src/data/guide-content/hanging-baskets.js` and `src/data/guide-content/container-vegetables.js` expanded from 2-paragraph stubs to full guides — the fifth stub-guide batch (Item 1), completing the plan's "container/small-space" grouping (balcony-garden, window-box, hanging-baskets, container-vegetables all now full guides).

| Guide | Sections | Notes |
|---|---|---|
| Hanging Basket Guide (`id: 'hanging-baskets'`) | 6 sections | Basket sizing/liner table, soil mix and planting density, full-sun combinations table, shade combinations table, watering/feeding schedules, preventing summer decline |
| Vegetables in Containers (`id: 'container-vegetables'`) | 6 sections | Minimum container sizes by crop table, best compact varieties table, potting mix, fertilizing schedule table, heat stress management, watering strategies to prevent blossom end rot |

- New theme entries in `GuideDetail.jsx`: `hanging-baskets` (fuchsia/purple gradient), `container-vegetables` (lime/yellow gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline. One image (`container-vegetables` light-colored-pots) needed a regenerate — the first attempt showed sealed plastic storage totes instead of actual plant containers, a content mismatch caught on review; reprompted explicitly for planter pots with visible vegetables and saucers, which fixed it. All 12 final images manually reviewed before commit (no gibberish text or artifacts).
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 65 full / 22 stubs (was 63/24)

### Balcony Garden & Window Box Guides Expanded to Full Content ✅ (2026-09-07)

`src/data/guide-content/balcony-garden.js` and `src/data/guide-content/window-box.js` expanded from 2-paragraph stubs to full guides — the fourth stub-guide batch (Item 1), covering the "container/small-space" grouping from the plan.

| Guide | Sections | Notes |
|---|---|---|
| Balcony Garden (`id: 'balcony-garden'`) | 6 sections | Weight/structural limits, lightweight growing media table, wind management, best plants by sun exposure table, drip irrigation/self-watering systems, vertical growing systems |
| Window Box Gardening (`id: 'window-box'`) | 6 sections | Thriller/filler/spiller design formula, drainage and soil, full-sun plant combinations table, shade plant combinations table, seasonal replanting calendar, fertilizing and care |

- New theme entries in `GuideDetail.jsx`: `balcony-garden` (sky/slate gradient), `window-box` (rose/orange gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 63 full / 24 stubs (was 61/26)

### Deer-Resistant & Pest-Eliminating Plants Guides Expanded to Full Content ✅ (2026-09-06)

`src/data/guide-content/deer-resistant.js` and `src/data/guide-content/pest-eliminating-plants.js` expanded from 2-paragraph stubs to full guides — the third stub-guide batch (Item 1).

| Guide | Sections | Notes |
|---|---|---|
| Deer-Resistant Plants (`id: 'deer-resistant'`) | 6 sections | Why resistance isn't proof, reliable perennials/bulbs table, reliable shrubs/trees table, repellent spray comparison, fencing/physical deterrents, landscape strategy |
| Pest-Eliminating Plants (`id: 'pest-eliminating-plants'`) | 4 sections | Scent-repellent plants, trap crops, border/hedge planting design, an honest evidence-strength table separating well-supported claims from folklore |

- Deliberately differentiated `pest-eliminating-plants` from the already-published `organic-pest-control` (product-focused: Bt, neem, DE) and `companion-planting` (pairing-table-focused) guides by centering it on plant selection and garden-design strategy, with explicit cross-references to both.
- New theme entries in `GuideDetail.jsx`: `deer-resistant` (amber/orange gradient), `pest-eliminating-plants` (green/teal gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 61 full / 26 stubs (was 59/28)

### Organic Pest Control & Slug Control Guides Expanded to Full Content ✅ (2026-09-06)

`src/data/guide-content/organic-pest-control.js` and `src/data/guide-content/slug-control.js` expanded from 2-paragraph stubs to full guides — the second stub-guide batch (Item 1), authored directly from compiled horticultural knowledge per the same standard as the companion-planting/soil-health batch.

| Guide | Sections | Notes |
|---|---|---|
| Organic Pest Control (`id: 'organic-pest-control'`) | 7 sections | IPM framework, physical barriers, Bt subspecies table, neem oil vs. insecticidal soap, diatomaceous earth + beneficial nematode species table, building beneficial-insect habitat, quick reference by common pest |
| Slug & Snail Control (`id: 'slug-control'`) | 6 sections | Slug/snail biology, bait options (iron phosphate vs. metaldehyde safety comparison), traps (beer/board), habitat reduction, natural predators, most/least vulnerable plants |

- New theme entries in `GuideDetail.jsx`: `organic-pest-control` (lime/emerald gradient), `slug-control` (slate/cyan gradient)
- 5 inline images + 1 hero image per guide, same Z-Image Turbo/ComfyUI pipeline as the first batch; all 12 manually reviewed before commit (no artifacts)
- Verified via full `npm run build` (368 static pages) and grep-confirmed 5 `<figure>` elements + correct paths in both guides' built HTML
- Guide counts updated: 59 full / 28 stubs (was 57/30)
...
- 4-stat grid: 185 Plants in database / 55 In-depth guides / 12 Plant categories / 100% Free, no account
...
- **`src/data/guide-content/index.js`** — 34 new imports and contentMap entries added; contentMap now covers 87 guides
...
**Guide counts:**
- 55 full in-depth guides (full section/table/tip/affiliate content) — includes No-Dig Gardening and Seed Saving, expanded from stubs
...
| `src/components/plants/PlantDatabase.jsx` | `'use client'` — the `/plants/` list. Live search (name + scientific name) plus filter chips for type/sun/water/lifecycle/experience/hydroponic-compatible, all client-side over the full 185-plant set (no pagination needed at this scale). Each card is a `PlantCard` sub-component showing an `aspect-square` `primary.jpg` thumbnail (type badge overlaid top-right, `onError` fallback to the plant's emoji — same pattern as `shop/ShopGrid.jsx`'s `ProductCard`) above the name/scientific name/description |
| `src/components/plants/PlantDetail.jsx` | Server component, same shape as `ProductDetail.jsx` (breadcrumb → photo gallery + header info → quick-facts grid → care notes → conditional pet-safety/medicinal callouts → cross-links → back link). Quick-facts grid merges the base `plants.js` fields (sun, water, zones, space, soil, seasons, experience) with the new profile fields |
...
**Photo gallery:** each detail page renders a real photo gallery — reusing `shop/ImageGallery.jsx` (main image + thumbnail strip + click-to-enlarge lightbox, already built for `/shop/`) rather than duplicating that logic — sourced from `public/plants/<id>/{primary,secondary-1,secondary-2,secondary-3}.jpg`. All 185 plant folders follow this exact 4-file naming convention (verified 1:1 against `plants.js` ids before wiring anything up — zero missing folders, zero orphaned folders, zero unexpected filenames), so `PlantDetail.jsx` builds the gallery path list directly from `plantId` with no per-plant data entry needed; `ImageGallery` already falls back to the plant's emoji per-image if a file is ever missing. The top section is a two-column `lg:grid-cols-2` layout (gallery left, badges/name/scientific name/description right), matching `ProductDetail.jsx`'s established pattern, stacking to a single column on mobile.
