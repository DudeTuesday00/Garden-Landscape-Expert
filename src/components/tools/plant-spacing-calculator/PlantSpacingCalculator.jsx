...
export default function PlantSpacingCalculator() {
  ...
  const colorByPlantId = Object.fromEntries(assignments.map((a) => [a.plantId, a.color]))
                ...
                {assignments.map((a) => (
                  <div key={a.plantId} className="flex items-center justify-between gap-3 border border-gray-200 dark:border-gray-700 rounded-xl px-3 py-2">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className={`w-3 h-3 rounded-sm flex-shrink-0 ${a.color}`} />
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 lg:sticky lg:top-20">
            ...
            {totalSquares > 0 && totalSquares <= 400 ? (
              <div
                ...
                style={{ gridTemplateColumns: `repeat(${Math.round(parseFloat(length)) || 1}, minmax(0, 1fr))` }}
              >
                {grid.flat().map((cell, i) => (
                  <div
                    key={i}
                    className={`aspect-square rounded-md flex items-center justify-center text-sm border border-gray-100 dark:border-gray-700 ${
                      cell ? colorByPlantId[cell.plantId] || 'bg-garden-100 dark:bg-garden-900/40' : 'bg-gray-50 dark:bg-gray-900'
                    }`}
                    title={cell ? cell.plantId : 'empty'}
                  >
                    {cell ? cell.emoji : ''}
                  </div>
                ))}
